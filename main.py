...
# проверка на выигрыш
# проверка на победные комбинации в строчках, столбцах или по диагонали
# arr - массив
...
async def newGame(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    data = st.SYMBOL_UNDEF * 9
    await update.message.reply_text(
        st.ANSW_YOUR_TURN,
        reply_markup=InlineKeyboardMarkup(getKeyboard(data))
)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if alert:
        ...
        await query.edit_message_text(
            text=message,
            reply_markup=InlineKeyboardMarkup(getKeyboard(callbackData))
        )
...
if __name__ == '__main__':
    application = ApplicationBuilder().token(getToken()).build()  # получения токена из файла 'token.txt' и инициализация updater
